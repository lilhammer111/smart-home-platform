...
struct UpdateProgramInfoReq {
    ...
    7: optional i32 FeederId (api.body="feeder_id");
    2: optional list<i32> Frequency (api.body="frequency");
    3: optional i16 FeedTime (api.body="feed_time");
    4: optional i16 Amount (api.body="amount");
    6: optional i8 Food (api.body="food");
}

struct AddProgramReq {
    7: required i32 FeederId (api.body="feeder_id", api.vd="$ > 0 && $ <= 2147483647;msg:Invalid Feeder Id.")
    ...
    3: required i16 FeedTime (api.body="feed_time", api.vd="($ / 100) < 24 && ($ / 100) > 0 && ($ % 100) < 60 ($ % 100) > 0;msg:Invalid Feed Time.");
    4: required i16 Amount (api.body="amount", api.vd="$ > 0 && $ <= 100 && $ % 10 == 0;msg:Invalid Amount.");
    6: required i8 Food (api.body="food", api.vd="$ >= 0 && $ <= 127;msg:Invalid Food Parameter.");
}
...
service RecordService {
    ...
    RecordInfo AddRecord(1: AddRecordReq req) (api.post="/api/feeder/record/post");
